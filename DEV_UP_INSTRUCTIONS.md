...
**Company lens (independent):** Replit (`replit`)  
**Innovation:** Workspace Cap Matrix  
**Scaffold batch:** 2026-08-10T0924Z

## Mission

Implement a **real, testable** central mechanism that addresses the bottleneck below. Do **not** claim Replit affiliation, proprietary access, or production deployment.
...
Dispatch workspace actions only through a capability matrix with budget and revoke.

## Hard rules (fail closed)

1. **No affiliation theater** — never state or imply Replit employment, endorsement, or proprietary systems access.
...
File: `src/workspace_cap_matrix.py`

- Expand `WorkspaceCapMatrix` into a complete, self-contained implementation.
- Public API must stay stable enough that tests in `tests/test_workspace_cap_matrix.py` can be upgraded (not gutted).
- Include at least:
...
### 3. Strengthen tests
Files: `tests/test_workspace_cap_matrix.py`, `tests/test_adversarial.py`
...
- Cloud deploy, customer pilots, proprietary Replit APIs
